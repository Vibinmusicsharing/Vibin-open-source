...
/**
 * The activity_youtube_2 that manages the login flow.
...
public class LoginActivity extends Activity implements AuthenticationClient.AuthenticationClientListener {
    ...
    private static final String NO_CALLER_ERROR = "Can't use LoginActivity with a null caller. " +
            "Possible reasons: calling activity_youtube_2 has a singleInstance mode " +
    ...
    public static Intent getAuthIntent(Activity contextActivity, AuthenticationRequest request) {
        if (contextActivity == null || request == null) {
            throw new IllegalArgumentException("Context activity_youtube_2 or request can't be null");
        }
    ...
    protected void onResume() {
        super.onResume();
        // User got back to the activity_youtube_2 from the auth flow
        if (mBackgrounded) {
            mBackgrounded = false;
            onClientCancelled();
        }
