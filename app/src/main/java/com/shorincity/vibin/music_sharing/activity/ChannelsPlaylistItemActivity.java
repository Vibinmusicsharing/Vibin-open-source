                ...
                public void onItemClick(View v, int position) {
                    try {
                        ...
                        for (int i = 0; i < playlistItemModel.getItems().size(); i++) {
                            if (playlistItemModel.getItems().get(i).getSnippet().getThumbnails().getMedium() != null) {
                                defaultThumbnail = playlistItemModel.getItems().get(i).getSnippet().getThumbnails().getMedium().getUrl();
                                break;
                            }
                        }
                        for (int i = 0; i < playlistItemModel.getItems().size(); i++) {
                            if (playlistItemModel.getItems().get(i).getSnippet().getThumbnails().getMedium() != null) {
                                playlist.add(new PlaylistDetailModel(
                                        ...
                                        playlistItemModel.getItems().get(i).getSnippet().getThumbnails().getMedium().getUrl(),
                                        playlistItemModel.getItems().get(i).getSnippet().getResourceId().getVideoId(),
                                        playlistItemModel.getItems().get(i).getSnippet().getSongDuration()
                                ));
                            } else {
                                playlist.add(new PlaylistDetailModel(
                                        playlistItemModel.getItems().get(i).getSnippet().getTitle(),
                        ...
                        if (playlistItemModel.getItems().get(position).getSnippet().getThumbnails().getMedium() != null) {
                            bundle.putString("thumbnail", playlistItemModel.getItems().get(position).getSnippet().getThumbnails().getMedium().getUrl());
                        } else {
