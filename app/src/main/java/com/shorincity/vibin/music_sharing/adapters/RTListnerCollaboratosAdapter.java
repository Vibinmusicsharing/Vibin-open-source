...
// view collab adapter
public class RTListnerCollaboratosAdapter extends RecyclerView.Adapter<RTListnerCollaboratosAdapter.ExampleViewHolder> {

    private Context mContext;
    private ArrayList<RTListner> mExampleList;
    private CustomItemClickListener callback;
    private boolean isAdmin;
    private int userId;

    public RTListnerCollaboratosAdapter(Context context, ArrayList<RTListner> exampleList, boolean isAdmin, int userId, CustomItemClickListener callback) {
        mContext = context;
    ...
    public void onBindViewHolder(@NonNull ExampleViewHolder holder, int position) {
        RTListner currentItem = mExampleList.get(position);
        if (currentItem != null) {
            String avatarUrl = currentItem.getUserAvatar();
            String fullname = currentItem.getUserName();
...
//            ivProfile.setOnClickListener(v -> callback.onItemClick(0, getAdapterPosition()));
            ivProfile.setOnLongClickListener(v -> {
                if (isAdmin && mExampleList.get(getAdapterPosition()).getUserId().intValue() != userId) {
